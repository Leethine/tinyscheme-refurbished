/* $Id: str_util.h,v 1.27 2025/07/12 20:39:44 alex Exp alex $ */
/*******************************************************************************

    str_util.h

    String Manipulation Functions.

*******************************************************************************/

#ifndef  STR_UTIL_H		/* Has the file been INCLUDE'd already? */
#define  STR_UTIL_H  yes

#ifdef __cplusplus		/* If this is a C++ compiler, use C linkage */
extern  "C"  {
#endif


#include  "pragmatics.h"		/* Compiler, OS, logging definitions. */

#include  <stddef.h>			/* Standard C definitions. */
#include  <string.h>			/* Standard C string functions. */

					/* Global debug switch (1/0 = yes/no). */
extern  int  str_util_debug  OCD ("str_util") ;


/*******************************************************************************
    Public functions (mine).
*******************************************************************************/

extern  errno_t  memdup2 P_((const void *block,
                             size_t length,
                             void **duplicate))
    OCD ("str_util") ;

extern  void  *memncpym P_((void *destination,
                            const void *source,
                            size_t length,
                            size_t maximum))
    OCD ("str_util") ;

extern  errno_t  strdup2 P_((const char *string,
                             char **duplicate))
    OCD ("str_util") ;

extern  char  *strncatm P_((char *destination,
                            const char *source,
                            size_t length,
                            size_t maximum))
    OCD ("str_util") ;

extern  char  *strncpym P_((char *destination,
                            const char *source,
                            size_t length,
                            size_t maximum))
    OCD ("str_util") ;

extern  errno_t  strndup2 P_((const char *string,
                              size_t length,
                              char **duplicate))
    OCD ("str_util") ;

extern  char  *strConvert P_((char *string))
    OCD ("str_util") ;

extern  errno_t  strDesmart P_((char *string,
                                ssize_t length,
                                ssize_t maxSize))
    OCD ("str_util") ;

extern  char  *strDestring P_((char *string,
                               ssize_t length,
                               const char *quotes))
    OCD ("str_util") ;

extern  size_t  strDetab P_((char *stringWithTabs,
                             ssize_t length,
                             int tabStops,
                             char *stringWithoutTabs,
                             size_t maxLength))
    OCD ("str_util") ;

extern  char  *strDupCat P_((ssize_t numStrings,
                             ...))
    OCD ("str_util") ;

extern  errno_t  strEnv P_((const char *string,
                            ssize_t length,
                            char *translation,
                            size_t maxLength))
    OCD ("str_util") ;

extern  char  *strEtoA P_((char *string,
                           ssize_t length))
    OCD ("str_util") ;

extern  errno_t  strInsert P_((const char *substring,
                               ssize_t subLength,
                               size_t offset,
                               char *string,
                               size_t maxLength))
    OCD ("str_util") ;

extern  bool  strMatch P_((const char *target,
                           const char *model,
                           bool ignoreCase))
    OCD ("str_util") ;

extern  size_t  strRemove P_((size_t numChars,
                              size_t offset,
                              char *string))
    OCD ("str_util") ;

extern  errno_t  strReplace P_((char *target,
                                ssize_t maxSize,
                                bool ignoreCase,
                                const char *find,
                                ssize_t findLen,
                                const char *replace,
                                ssize_t replaceLen,
                                ssize_t maxReplace,
                                size_t *numReplace))
    OCD ("str_util") ;

extern  size_t  strTrim P_((char *string,
                            ssize_t length))
    OCD ("str_util") ;

extern  size_t  strTrimNL P_((char *string,
                              ssize_t length))
    OCD ("str_util") ;

/*******************************************************************************
    Public functions (non-standard, but commonly found in other libraries).
*******************************************************************************/

#if !defined(HAVE_MEMDUP) || !HAVE_MEMDUP
    extern  void  *memdup P_((const void *source,
                              size_t length))
        OCD ("str_util") ;
#endif

#if !defined(HAVE_MEMXLT) || !HAVE_MEMXLT
    extern  void  *memxlt P_((void *block,
                              const char *table,
                              size_t length))
        OCD ("str_util") ;
#endif

#if defined(HAVE_STPCPY) && !HAVE_STPCPY
    extern  char  *stpcpy P_((char *destination,
                              const char *source))
        OCD ("str_util") ;
#endif

#if defined(HAVE_STRCASECMP) && !HAVE_STRCASECMP
    extern  int  strcasecmp P_((const char *thisString,
                                const char *thatString))
        OCD ("str_util") ;
#endif

#if !defined(HAVE_STRCASESTR) || !HAVE_STRCASESTR
    extern  char  *strcasestr P_((const char *string,
                                  const char *substring))
        OCD ("str_util") ;
#endif

#if defined(HAVE_STRCSPN) && !HAVE_STRCSPN
    extern  size_t  strcspn P_((const char *string,
                                const char *reject))
        OCD ("str_util") ;
#endif

#if defined(HAVE_STRDUP) && !HAVE_STRDUP
    extern  char  *strdup P_((const char *string))
        OCD ("str_util") ;
#endif

#if defined(HAVE_STRLCAT) && !HAVE_STRLCAT
    extern  size_t  strlcat P_((char *destination,
                                const char *source,
                                size_t length))
        OCD ("str_util") ;
#endif

#if defined(HAVE_STRLCPY) && !HAVE_STRLCPY
    extern  size_t  strlcpy P_((char *destination,
                                const char *source,
                                size_t length))
        OCD ("str_util") ;
#endif

#if !defined(HAVE_STRLWR) || !HAVE_STRLWR
    extern  char  *strlwr P_((char *string))
        OCD ("str_util") ;
#endif

#if defined(HAVE_STRNCASECMP) && !HAVE_STRNCASECMP
    extern  int  strncasecmp P_((const char *thisString,
                                 const char *thatString,
                                 size_t length))
        OCD ("str_util") ;
#endif

#if !defined(HAVE_STRNCASESTR) || !HAVE_STRNCASESTR
    extern  char  *strncasestr P_((const char *string,
                                   const char *substring,
                                   size_t length))
        OCD ("str_util") ;
#endif

extern  char  *strnchr P_((const char *string,
                           int c,
                           size_t length))
    OCD ("str_util") ;

#if defined(HAVE_STRNDUP) && !HAVE_STRNDUP
extern  char  *strndup P_((const char *string,
                           size_t length))
    OCD ("str_util") ;
#endif

extern  char  *strndupx P_((const char *string,
                            size_t length,
                            size_t extra))
    OCD ("str_util") ;

#if defined(HAVE_STRNLEN) && !HAVE_STRNLEN
extern  size_t  strnlen P_((const char *string,
                            size_t maxLength))
    OCD ("str_util") ;
#endif

#if !defined(HAVE_STRNLWR) || !HAVE_STRNLWR
    extern  char  *strnlwr P_((char *string,
                               size_t length))
        OCD ("str_util") ;
#endif

#if !defined(HAVE_STRNSTR) || !HAVE_STRNSTR
    extern  char  *strnstr P_((const char *string,
                               const char *substring,
                               size_t length))
        OCD ("str_util") ;
#endif

#if !defined(HAVE_STRNUPR) || !HAVE_STRNUPR
    extern  char  *strnupr P_((char *string,
                               size_t length))
        OCD ("str_util") ;
#endif

#if defined(HAVE_STRRCHR) && !HAVE_STRRCHR
    extern  char  *strrchr P_((const char *string,
                               int c))
        OCD ("str_util") ;
#endif

#if defined(HAVE_STRSPN) && !HAVE_STRSPN
    extern  size_t  strspn P_((const char *string,
                               const char *accept))
        OCD ("str_util") ;
#endif

#if defined(HAVE_STRTOK) && !HAVE_STRTOK
    extern  char  *strtok P_((char *string,
                              const char *delimiters))
        OCD ("str_util") ;
#endif

#if defined(HAVE_STRTOK_R) && !HAVE_STRTOK_R
    extern  char  *strtok_r P_((char *string,
                                const char *delimiters,
                                char **context))
        OCD ("str_util") ;
#endif

#if !defined(HAVE_STRUPR) || !HAVE_STRUPR
    extern  char  *strupr P_((char *string))
        OCD ("str_util") ;
#endif

#if !defined(HAVE_STRXLT) || !HAVE_STRXLT
    extern  char  *strxlt P_((char *string,
                              const char *table))
        OCD ("str_util") ;
#endif

#if !defined(HAVE_XLTABLE) || !HAVE_XLTABLE
    extern  char  *xltable P_((char *table,
                               const char *from,
                               const char *to))
        OCD ("str_util") ;

    extern  char  *xltable1 P_((char *table,
                                const char *fromto))
        OCD ("str_util") ;
#endif


#ifdef __cplusplus		/* If this is a C++ compiler, use C linkage */
}
#endif

#endif				/* If this file was not INCLUDE'd previously. */
