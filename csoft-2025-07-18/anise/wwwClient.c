/* $Id: wwwClient.c,v 1.7 2016/08/01 02:12:18 alex Exp alex $ */
/*******************************************************************************
...
typedef  struct  _WwwClient {
    ...
    const  char  *tildeTranslation ;	/* Format string for replacing '~'. */
...
Purpose:
    ...
    Invocation:
        ...
        status = wwwClientCreate (connection, dispatcher, tuple, &client) ;
    ...
    where:
        ...
        <dispatcher>	- I
            ...
            will be registered.
        <tuple>		- I
            This function, wwwClientCreate(), is actually a callback function
            called from the private function portAnswer() in the PORT_UTIL
            package.  As such, it must conform to the ClientCreateFunc type
            signature defined in "port_util.h", which only allows one data
            parameter from the application.  This function requires two
            data parameters, so main() (in "anise.c") creates a tuple of
            the two parameters and the tuple gets passed to wwwClientCreate().
            The two parameters are:
                <tildeTranslation>	- I
                    specifies a format string used for tilde translation.
                    The string is of the form, "...%s...%s...", where the
                    "%s"s are replaced by the user's name and the trailing
                    portion of the resource name, respectively.
                <logFile>		- I
                    is the LogFile handle for the file to which HTTP
                    transactions are to be logged; NULL can be specified
                    if transactions are not to be logged.
        <client>	- O
...
#    if PROTOTYPES
        ...
        WwwClient  *client)
#    else
        connection, context, tuple, client)
...
{    /* Local variables. */
    ...
    if (connection == NULL) {
        SET_ERRNO (EINVAL) ;
        LGE "(wwwClientCreate) NULL TCP/IP connection handle.\n") ;
        return (errno) ;
    } else if (dispatcher == NULL) {
        SET_ERRNO (EINVAL) ;
        LGE "(wwwClientCreate) NULL I/O dispatcher handle.\n") ;
        return (errno) ;
    }

/* Get the client data parameters from the tuple. */

    tildeTranslation = (char *) tplGet (tuple, 0) ;
    if (tildeTranslation == NULL)  tildeTranslation = "%s/html/%" ;
    logFile = (LogFile) tplGet (tuple, 1) ;
    /* Don't destroy the tuple --
       portAnswer() needs to pass it to subsequent clients!!! */
...
/* Create and initialize a client object. */
    ...
    *client = (WwwClient) malloc (sizeof (_WwwClient)) ;
    ...
    (*client)->tildeTranslation = tildeTranslation ;
...
/* Evaluate the complete command. */
        ...
        if (httpEvaluate (client->stream, numLines, command,
                          NULL, client->tildeTranslation,
                          &keepAlive, &response)) {
