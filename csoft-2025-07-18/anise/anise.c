/* $Id: anise.c,v 1.11 2022/12/02 22:27:03 alex Exp alex $ */
/*******************************************************************************
...
Purpose:
    ...
    Invocation (UNIX):
        ...
        % anise [-aperror] [-daemon] [-debug] [-log <file>]
                ...
                [-ftp <cport>[,<dport>]] [-pass <port>] [-www <port>]
    ...
    where
        ...
        "-log <file>"
            enables logging of incoming HTTP requests to a file.  This file
            will be used to log all transactions on WWW ports which are
            specified (via the "-www" option) AFTER the "-log" option.  If
            the "-log" option has NOT been specified prior to specifying a
            port, the port's transactions will NOT be logged.  Different log
            files can be used for different ports by simply interspersing
        ...
        "-tilde <format>"
            ...
            respectively.
        "-ftp <cport>[,<dport>]"
            specifies (i) a network server port (i.e., service name or port
            number) at which ANISE will listen for control connection requests
            from FTP clients and (ii) a default port for data connections.
            Multiple server ports can be monitored for control connection
            requests by repeating this option for each different port.
            Connections established via the FTP control port, "<cport>",
            are used to exchange FTP commands and responses between the
            client and server.  Connections established via the FTP data
            port, "<dport>", are used to exchange data (e.g., file contents)
            between the client and server; if not specified, the data port
            defaults to the standard "ftp-data" port (20).  The data port
            capability is used in "active" mode, wherein the FTP server
            connects to the data port of the client in order to exchange
            data.  This mode of operation is rarely used now for security
            reasons and I haven't been able to test ANISE's command-line
            specification of the default data port as a result.  Most FTP
            clients put the server in "passive" mode; instead of initiating
            the data connection with the client, the FTP server now listens
            at a temporary, system-allocated port for a data connection
...
{  /* Local variables. */
    bool  isDaemon ;
    char  *argument, *ctrlPort, *dataPort, *pathname ;
    char  *target, *tildeTranslation ;
...
/* Construct a default tilde translation format string.  This can be
   overridden by specifying the "-tilde <format>" option *before* the
   "-www <port>" option. */

    tildeTranslation = getenv ("TILDE_TRANSLATION") ;
    if (tildeTranslation == NULL) {
        pathname = fnmBuild (FnmDirectory, "~", NULL) ;
        if (pathname == NULL) {
            LGE "[%s] Error parsing home directory in tilde translation.\nfnmBuild: ",
                argv[0]) ;
            exit (errno) ;
        }
        tildeTranslation = strDupCat (2, pathname, "%s/html/%s") ;
        if (tildeTranslation == NULL) {
            LGE "[%s] Error parsing concatenating tilde translation.\nstrDupCat: ",
                argv[0]) ;
            exit (errno) ;
        }
        free (pathname) ;
    ...
    while ((option = opt_get (scan, &argument))) {
        ...
        case 3:			/* "-debug" */
            ...
            aperror_save = 1 ;
            break ;
        case 4:			/* "-ftp <cport>[,<dport>]" */
            ctrlPort = strtok (argument, " ,") ;
            dataPort = strtok (NULL, " ,") ;
            if (portCreate (ctrlPort, dispatcher,
    ...
    if (errflg) {
        ...
        fprintf (stderr, "              [-ftp <cport>[,<dport>]] [-pass <port>] [-www <port>]\n") ;
        exit (EINVAL) ;
    }

/*******************************************************************************
