/* $Id: skt_util.c,v 1.23 2019/02/22 06:58:36 alex Exp alex $ */
/*******************************************************************************
...
/* Configure the socket with the desired I/O mode. */
    ...
    if (IOCTLSOCKET (fd, FIONBIO, &optval) == -1) {
        ...
        LGE "(sktBlock) Error configuring socket %ld for %s I/O.\nioctl: ",
            (long) fd, blocking ? "blocking" : "non-blocking") ;
        return (errno) ;
    }
...
/* Poll the data socket for input. */
    ...
    for ( ; ; ) {
        ...
        if (select ((int) fd+1, &readMask, NULL, NULL, &timeout) >= 0)  break ;
        SET_ERRNO (GET_NETERRNO ()) ;
        if (errno == EINTR)  continue ;		/* Retry on signal interrupt. */
        LGE "(sktIsReadable) Error polling socket %ld.\nselect: ", (long) fd) ;
        return (false) ;
    }
						/* No input pending? */
...
/* Input is pending.  Find out how many bytes of data are actually available
   ...
   zero bytes of pending input, the connection is broken. */
    ...
    if (length > 0) {
        ...
        SET_ERRNO (EPIPE) ;
        LGE "(sktIsReadable) Broken connection to socket %ld.\n", (long) fd) ;
        return (false) ;			/* EOF. */
    }
...
/* Poll the data socket for input. */
    ...
    for ( ; ; ) {
        ...
        SET_ERRNO (GET_NETERRNO ()) ;
        if (errno == EINTR)  continue ;		/* Retry on signal interrupt. */
        LGE "(sktIsUp) Error polling socket %ld.\nselect: ", (long) fd) ;
...
/* Input is pending.  Find out how many bytes of data are actually available
   ...
   zero bytes of pending input, the connection is broken. */
    ...
    if (length > 0) {				/* Pending input? */
        ...
        SET_ERRNO (EPIPE) ;
        LGE "(sktIsUp) Broken connection to socket %ld.\n", (long) fd) ;
        return (false) ;			/* Connection is down. */
    }
...
/* Poll the data socket for output. */
    ...
    for ( ; ; ) {
        ...
        timeout.tv_sec = timeout.tv_usec = 0 ;	/* No wait. */
        if (select ((int) fd+1, NULL, &writeMask, NULL, &timeout) >= 0)  break ;
        SET_ERRNO (GET_NETERRNO ()) ;
        if (errno == EINTR)  continue ;		/* Retry on signal interrupt. */
        LGE "(sktIsWriteable) Error polling socket %ld.\nselect: ", (long) fd) ;
        return (false) ;
    }
						/* Ready for output? */
   ...
   to be read. */
    ...
    if (buffer == NULL) {
        ...
        while (IOCTLSOCKET (fd, FIONREAD, &length) == -1) {
            SET_ERRNO (GET_NETERRNO ()) ;
            if (errno == EINTR)  continue ;	/* Retry on signal interrupt. */
            LGE "(sktPeek) Error polling socket %ld.\nioctl: ", (long) fd) ;
            return (errno) ;
        }
        if (numBytes != NULL)  *numBytes = (size_t) length ;
        return (0) ;
#endif
    }
...
/* Peek at the pending input data. */
    ...
    byteCount = (ssize_t) recv (fd, buffer, maxBytes, MSG_PEEK) ;
    if (byteCount < 0) {
        SET_ERRNO (GET_NETERRNO ()) ;
        LGE "(sktPeek) Error peeking at socket %ld's pending input.\nrecv: ",
            (long) fd) ;
        return (errno) ;
    }
...
/* Get the IP address of the host on the other end of the network connection. */
    ...
    if (getpeername (fd, &peerAddress, &length)) {
        SET_ERRNO (GET_NETERRNO ()) ;
        LGE "(net_peer) Error getting peer's host for socket %ld.\ngetpeername: ",
            (long) fd) ;
        return (NULL) ;
    }
...
Purpose:
    ...
    where
        ...
        <fd>	- I
            is the UNIX file descriptor for the socket.
        <port>	- O
            returns the socket's port number.  In case of an error, ERRNO
            is set and zero (0) is returned as the port number.
...
/* Get the socket's port number and convert it to host byte ordering. */
    ...
    if (getsockname (fd, (struct sockaddr *) &socketName, &length)) {
        SET_ERRNO (GET_NETERRNO ()) ;
        LGE "(sktPort) Error getting port number for socket %ld.\ngetsockname: ",
            (long) fd) ;
        return (0) ;
    }

    return ((in_port_t) ntohs (socketName.sin_port)) ;
...
Purpose:
    ...
    where
        ...
        <receiveSize>	- I
            ...
            If this argument is zero or less, the receive buffer retains
            its current size.
        <sendSize>	- I
            specifies the new size of the socket's send system buffer.  If
            this argument is zero or less, the send buffer retains its
            current size.
        <status>	- O
...
/* Change the size of the socket's receive system buffer. */
    ...
    if ((receiveSize > 0) &&
        setsockopt (fd, SOL_SOCKET, SO_RCVBUF,
                    (void *) &receiveSize, sizeof (socklen_t))) {
        LGE "(sktSetBuf) Error setting receive buffer size (%lu) for socket %ld.\nsetsockopt: ",
            (unsigned long) receiveSize, (long) fd) ;
        return (errno) ;
    }
...
/* Change the size of the socket's send system buffer. */
    ...
    if ((sendSize > 0) &&
        setsockopt (fd, SOL_SOCKET, SO_SNDBUF,
                    (void *) &sendSize, sizeof (socklen_t))) {
        LGE "(sktSetBuf) Error setting send buffer size (%lu) for socket %ld.\nsetsockopt: ",
            (unsigned long) sendSize, (long) fd) ;
        return (errno) ;
    }
...
#    ifndef PALM_PILOT_HOST_NAME
#        define  PALM_PILOT_HOST_NAME  "tara.home"
#    endif
    if (sethostname (PALM_PILOT_HOST_NAME, strlen (PALM_PILOT_HOST_NAME)))
        return (errno) ;
#endif
    return (0) ;
}
