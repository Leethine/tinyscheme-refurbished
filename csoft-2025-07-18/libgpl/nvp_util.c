/* $Id: nvp_util.c,v 1.14 2021/09/20 02:14:02 alex Exp $ */
/*******************************************************************************
...
typedef  struct  _NVPair {
    ...
    size_t  numElements ;		/* 1 for scalar, N for array. */
...
Purpose:
    ...
    where
        ...
        <count>	- O
            ...
            string for a string value.  0 is returned if an invalid pair
...
{    /* Local pairs. */
    char  *name, *s, *value ;
    const  char  *fieldStart, *matchStart ;
    double  real ;
    int  status ;
    long  integer ;
    NVList  list ;
    NVPair  pair ;
    NvpDataType  dataType ;
    size_t  fieldLength, i, matchLength, numElements ;
    static  CompiledRE  pattern  OCD ("nvp_util")  = NULL ;
    static  MatchedRE  match  OCD ("nvp_util")  = NULL ;
...
#define  SPEC_PATTERN	\
    ...
    if ((pattern == NULL) && rexCreate (SPEC_PATTERN, &pattern, &match)) {
        LGE "(nvpDecode) Error compiling regular expression: \"%s\"\nrexCreate: ",
            SPEC_PATTERN) ;
        pattern = NULL ;  match = NULL ;
        return (NULL) ;
    }

/* Parse the left side of the specification. */

    if (!rexMatch (spec, -1, pattern, match, &matchStart, &matchLength)) {
        SET_ERRNO (EINVAL) ;
...
/* Duplicate the name. */
    ...
    rexGetGroup (match, 1, &fieldStart, &fieldLength) ;
    name = strndup (fieldStart, fieldLength) ;
    if (name == NULL) {
...
/* Lookup the data type, if it was specified. */
    ...
    rexGetGroup (match, 2, &fieldStart, &fieldLength) ;
    if (fieldStart != NULL) {
        for (i = 0 ;  typeInfo[i].name != NULL ;  i++) {
            if (strncasecmp (typeInfo[i].name, fieldStart, fieldLength) == 0)
                break ;
        }
...
/* Process the array size, if it was specified. */
    ...
    numElements = 1 ;
    rexGetGroup (match, 3, &fieldStart, &fieldLength) ;
    if (fieldStart != NULL) {
        numElements = (size_t) strtoul (fieldStart, &s, 0) ;
        if ((s != (fieldStart + fieldLength)) || (numElements < 1)) {
            SET_ERRNO (EINVAL) ;
...
/* Advance to the value field in the specification. */
    ...
    value = (char *) matchStart + matchLength ;
...
{    /* Local pairs. */
    ...
    if ((pair->dataType == NvpUndefined) || (pair->value == NULL)) {
        ...
        if (typeInfo[i].name == NULL)
            ...
            sprintf (spec, "%s(%s[%lu]) = %s",
                     nvpName (pair), typeInfo[i].name,
                     (unsigned long) pair->numElements, nvpString (pair)) ;
        else
            sprintf (spec, "%s(%s) = %s",
                     nvpName (pair), typeInfo[i].name,
                     nvpString (pair)) ;
        ...
        case NvpByte:
          { unsigned  char  *array = NULL ;
            ...
            if (length == 1) {
                ...
                xdrStream->x_op = XDR_DECODE ;
            } else if (nvpAssign (*pair, (size_t) length,
                                  dataType, array, NvpDynamic)) {
                return (FALSE) ;
            }
            break ;
          }
...
/* Encode the value. */
        ...
        case NvpByte:
          { unsigned  char  *array = nvpValue (*pair) ;
            u_int  length = (u_int) nvpCount (*pair) ;
            if (!xdr_bytes (xdrStream, (char **) &array, &length, UINT_MAX))
                return (FALSE) ;
            return (TRUE) ;
          }
