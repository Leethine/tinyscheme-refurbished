/* $Id: nft_proc.c,v 1.24 2022/12/02 03:25:29 alex Exp alex $ */
/*******************************************************************************
...
/* Build and save the new directory pathname. */
    ...
    if (newDirectory == NULL) {
        LGE "(nftCWD) Error parsing directory name: \"%s\"\nfnmBuild: ",
            argument) ;
    }
    PUSH_ERRNO ;
    free (argument) ;
    ...
    POP_ERRNO ;
    if (newDirectory == NULL)  return (errno) ;
    info->currentDirectory = newDirectory ;
...
{    /* Local variables. */
    ...
    if (fileName == NULL) {
        LGE "(nftFileCmds) Error parsing file name: \"%s\"\nfnmBuild: ",
            arguments) ;
        PUSH_ERRNO ;
        nftPutLine (session, "451 %s: %s\n", arguments, STRERROR (errno)) ;
        POP_ERRNO ;
        return (errno) ;
    }
    ...
    if (strcmp (command, "DELE") == 0) {
        ...
        if (stat (fileName, &fileInfo)) {
            ...
            time_t  mtime = (time_t) fileInfo.st_mtime ;
            nftPutLine (session, "213 %s", ctime (&mtime)) ;
        }
        ...
        free (fileName) ;
        fileName = fnmBuild (FnmPath, arguments, info->oldPathname, NULL) ;
        if (fileName == NULL) {
            LGE "(nftFileCmds) Error parsing file name: \"%s\"\nfnmBuild: ",
                arguments) ;
            PUSH_ERRNO ;
...
/* Construct the wildcard pathname. */
    ...
    if (arguments == NULL)
        pathname = fnmBuild (FnmPath, WILDCARD,
                             info->currentDirectory, NULL) ;
    else
        pathname = fnmBuild (FnmPath, arguments, WILDCARD,
                             info->currentDirectory, NULL) ;

    if (pathname == NULL) {
        LGE "(nftListCmds) Error parsing file name: \"%s\"\nfnmBuild: ",
            (arguments == NULL) ? WILDCARD : arguments) ;
        PUSH_ERRNO ;
        nftPutLine (session, "451 %s: %s\n",
                    (arguments == NULL) ? WILDCARD : arguments,
                    STRERROR (errno)) ;
   ...
   the speed of the transfer. */
    ...
    while (nextFile != NULL) {
      ...
      { char  *s = strrchr (nextFile, '/') ;
        ...
        fileName = strdup (s) ;
        if (fileName == NULL) {
            LGE "(nftListCmds) Error duplicating file name: \"%s\"\nstrdup: ",
                s) ;
        }
      }
#else
        fileName = fnmBuild (FnmFile, nextFile, NULL) ;
        if (fileName == NULL) {
            LGE "(nftFileCmds) Error parsing file name: \"%s\"\nfnmBuild: ",
                nextFile) ;
        }
        ...
        if (fullList && !stat (nextFile, &fileInfo)) {
...

{
    ...
    *arguments = toupper (INT_UCHAR (*arguments)) ;
...
/* Open the file being retrieved. */
    ...
    pathname = fnmBuild (FnmPath, arguments, info->currentDirectory, NULL) ;
    if (pathname == NULL) {
        LGE "(nftRETR) Error parsing pathname: \"%s\"\nfnmBuild: ",
            arguments) ;
        PUSH_ERRNO ;
        nftPutLine (session, "451 %s: %s\n", arguments, STRERROR (errno)) ;
        free (pathname) ;
    ...
    if (fileName == NULL) {
        LGE "(nftRETR) Error parsing file name: \"%s\"\nfnmBuild: ",
            pathname) ;
        PUSH_ERRNO ;
        nftPutLine (session, "451 %s: %s\n", arguments, STRERROR (errno)) ;
        fclose (file) ;
...
/* Establish a data connection with the client. */
    ...
    if (nftIsUp (session, 1)) {
        nftPutLine (session, "125 FILE: %s (%"PRIuSIZE_T" bytes) on %s [%c]\n",
                    fileName, (size_t) fileInfo.st_size, info->dataPortName,
        ...
        nftPutLine (session, "150 FILE: %s (%"PRIuSIZE_T" bytes) on %s [%c]\n",
                    fileName, (size_t) fileInfo.st_size, info->dataPortName,
                    info->representation[0]) ;
    }
...
/* Close the data connection. */
    ...
    nftPutLine (session, "226 RETR complete: %s (%"PRIuSIZE_T" bytes)\n",
                fileName, totalBytes) ;
    nftClose (session) ;
    fclose (file) ;
    free (fileName) ;
...
/* In the case of the Store Unique (STOU) command, generate a unique file
   name for the file being stored. */
    ...
    if (storeUnique) {
        ...
        for ( ; ; ) {
            if (!fnmExists (fnmPath (fname)))  break ;
            fnmDestroy (fname) ;
    ...
    if (fileName == NULL) {
        LGE "(nftStoreCmds) Error parsing file name: \"%s\"\nfnmBuild: ",
            storeUnique ? "<tmpnam>" : arguments) ;
        PUSH_ERRNO ;
        nftPutLine (session, "451 %s: %s\n", arguments, STRERROR (errno)) ;
        POP_ERRNO ;
        return (errno) ;
...
/* Close the data connection. */
    ...
    nftPutLine (session, "226 %s complete: %s (%"PRIuSIZE_T" bytes)\n",
...
/* Examine the second argument, the format parameter. */
    ...
    *format = toupper (INT_UCHAR (*format)) ;
