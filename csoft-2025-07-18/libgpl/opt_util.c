/* $Id: opt_util.c,v 1.19 2023/04/04 12:40:05 alex Exp alex $ */
/*******************************************************************************
...
Purpose:
    ...
    UNIX-style command line arguments.  The OPT_UTIL utilities support:
        ...
        (5) redirection of standard input and output on systems that
            ...
            (I/O redirection requires that C Preprocessor macro
            OPT_UTIL_IO_REDIRECTION be #define'd as a non-zero value
            when this source file is compiled.  I/O redirection must
            be specified on the command line as two separate arguments,
            the angle bracket and the file name.)
...
/* Check for I/O redirection, indicated by "<" (input) or ">" (output)
   characters.  NOTE that the angle bracket must be its own separate
   argument and that the file is the following argument; e.g.,
   '?command? > ?filename?'.  The angle bracket can NOT be flush up
   against the file name; e.g., the shell-like '?command? >?filename?'
   is not allowed.  This is necessary so that non-option arguments can
   begin with an angle bracket, although these arguments must be LONGER
   than the one-character angle bracket. */

#if defined(OPT_UTIL_IO_REDIRECTION) && OPT_UTIL_IO_REDIRECTION
...
/* If an argument is expected (because the option expects one or if I/O is
   ...
   options, the argument may be flush up against the option (i.e., the
   argument is the remainder of the current ARGV) or it may be separated
   from the option by white space (i.e., the argument is the whole of the
   next ARGV).  For name options, the argument is always found in the next
   ARGV. */
...
{    /* Local variables. */
    char  *name, *s ;
    int  position ;
    size_t  i, length, numLetters, numStrings ;
    ...
    if (argv != NULL) {
        ...
        if (scan->numNames > 0) {
            ...
            for (i = 0 ;  i < argc ;  i++)	/* Tabulate argument lengths. */
                scan->arglen[i] = (int) strlen (argv[i]) ;
        }
