/* $Id: coli_util.c,v 1.18 2019/05/28 03:53:59 alex Exp alex $ */
/*******************************************************************************
...
/* Encapsulate the code set information in an octet sequence.  The layout of
   ...
   structure (see section 13.10.2.4 in the CORBA specification). */
    ...
    LGI "(coliMakeIOR) Made %u@%s IOR with %"PRIuSIZE_T"-octet object key.\n",
...
/* Generate the stringified reference, which is simply a hex dump of
   the octet sequence encapsulation of the encoded IOR. */
    ...
    meoToHex (encapsulation.elements, encapsulation.count,
              iorString + strlen (iorString), true) ;
...
/* Build the IIOP message header. */
    ...
    header.message_size = (unsigned long) comxSkip (mxchan, 0, 0) ;
...
/* Convert the remainder of the string to a sequence of binary octets. */
    ...
    numBytes = meoFromHex (s, -1, rawIOR.elements) ;
    if (numBytes < rawIOR.count) {
        SET_ERRNO (EINVAL) ;
        ...
        if (key != NULL) {
            ...
            for ( ; ; ) {		/* Translate "%xx" escape sequences. */
                *t = (octet) *s ;
                if (*s == '\0')  break ;
                if (*s++ == '%') {
                    if (!isxdigit (INT_UCHAR (s[0])) ||
                        !isxdigit (INT_UCHAR (s[1]))) {
                        SET_ERRNO (EINVAL) ;
                        LGE "(coliURL2O) Invalid hexadecimal coding beginning near \"%s\".\n",
                            s) ;
                        goto onError ;
                    }
                    *t = isdigit (INT_UCHAR (s[0]))
                         ? (INT_UCHAR (s[0]) - INT_UCHAR ('0'))
                         : (toupper (INT_UCHAR (s[0])) - INT_UCHAR ('A') + 10) ;
                    *t = (*t << 4)
                         |
                         isdigit (INT_UCHAR (s[1]))
                         ? (INT_UCHAR (s[1]) - INT_UCHAR ('0'))
                         : (toupper (INT_UCHAR (s[1])) - INT_UCHAR ('A') + 10) ;
                    s += 2 ;
                }
                t++ ;
            }
