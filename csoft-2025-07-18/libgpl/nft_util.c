/* $Id: nft_util.c,v 1.22 2022/12/02 03:25:29 alex Exp $ */
/*******************************************************************************
...
/*******************************************************************************
    FTP Command Map - maps a command name to its command-processing function.
*******************************************************************************/

typedef  struct  NftCommandMap {
    char  *command ;
    NftCommandProc  *function ;
}  NftCommandMap ;
...
#define  MAXCOMMANDS  64
    size_t  numCommands ;		/* # of command-to-function maps. */
    NftCommandMap  *commandProcs ;	/* Maps FTP commands to functions. */
...
Purpose:
    ...
    Invocation:
        ...
        status = nftCreate (controlPoint, dataPort, commands, callbacks,
                            userData, &session) ;
    ...
    where
        ...
        <dataPort>	- I
            specifies the default service for FTP data connections; if this
            argument is NULL, the standard FTP data port, "ftp-data" (port 21),
            is used.
        <commands>	- I
...
#    if PROTOTYPES
        ...
        NftSession  *session)
#    else
        controlPoint, dataPort, commands, callbacks, userData, session)
...
/* Construct the default server address ("<port>@<peer>") for FTP data ports. */

    if (dataPort == NULL)  dataPort = "ftp-data" ;
    peer = (char *) sktPeer (tcpFd (controlPoint)) ;

    length = strlen (dataPort) ;
    if (peer != NULL)  length += strlen ("@") + strlen (peer) ;

    (*session)->info.dataPortName = malloc (length + 1) ;
    if ((*session)->info.dataPortName == NULL) {
        LGE "(nftCreate) Error allocating data port name for %s.\nmalloc: ",
            nftName (*session, 0)) ;
        PUSH_ERRNO ;  nftDestroy (*session) ;  POP_ERRNO ;
        return (errno) ;
    }

    strcpy ((*session)->info.dataPortName, dataPort) ;
    if (peer != NULL) {
        strcat ((*session)->info.dataPortName, "@") ;
        strcat ((*session)->info.dataPortName, peer) ;
    }

/* Allocate a table to map FTP command names to processing functions. */

    (*session)->commandProcs = malloc (MAXCOMMANDS * sizeof (NftCommandMap)) ;
    if ((*session)->commandProcs == NULL) {
...
/* Remember the current directory. */
    ...
    (*session)->info.currentDirectory = fnmBuild (FnmPath, NULL) ;
    if ((*session)->info.currentDirectory == NULL) {
        LGE "(nftCreate) Error getting current directory name for %s.\nfnmBuild: ",
            nftName (*session, 0)) ;
        PUSH_ERRNO ;  nftDestroy (*session) ;  POP_ERRNO ;
        return (errno) ;
    }
...
/* Delete the array of command-to-function maps. */

    if (session->commandProcs != NULL) {
        for (i = 0 ;  i < session->numCommands ;  i++) {
            if (session->commandProcs[i].command != NULL) {
                free (session->commandProcs[i].command) ;
                session->commandProcs[i].command = NULL ;
            }
        }
        free (session->commandProcs) ;
        session->commandProcs = NULL ;
    }
...
/* Lookup and execute the processing function for the command. */
    ...
    callback = nftLookup (session, session->command) ;
    if (callback == NULL) {
    ...
    return ((*callback) (session, session->command, session->arguments,
...
Procedure:
    ...
    Look Up the Function to Process an FTP Command.


Purpose:

    Function nftLookup() returns the function registered to process an FTP
    command.


    Invocation:

        callback = nftLookup (session, command) ;
    ...
    where
        ...
        <callback>	- O
            returns a pointer to the function used to process the specified
            FTP command.  NULL is returned if the command is not found in
            the session's command table or if NULL was explicitly registered
            as the callback for the command.

*******************************************************************************/


NftCommandProc  *nftLookup (
...
{    /* Local variables. */
    ...
    if (session == NULL) {
        SET_ERRNO (EINVAL) ;
        LGE "(nftLookup) NULL session handle:") ;
        return (NULL) ;
    } else if (command == NULL) {
        SET_ERRNO (EINVAL) ;
        LGE "(nftLookup) NULL command:") ;
        return (NULL) ;
    }

/* Step through the array of command-function maps looking for the command. */

    for (i = 0 ;  i < session->numCommands ;  i++) {
        if (strcmp (command, session->commandProcs[i].command) == 0)
            return (session->commandProcs[i].function) ;
    }

    SET_ERRNO (EINVAL) ;
    LGE "(nftLookup) Invalid command: \"%s\"\n", command) ;
    return (NULL) ;
...
/* Step through the array of command-function maps looking for the command. */
    ...
    for (i = 0 ;  i < session->numCommands ;  i++) {
        if (strcmp (command, session->commandProcs[i].command) == 0)  break ;
    }

/* If the command was found in the array, replace its processing function
   with the new function.  Otherwise, add the command-function map to the
   array. */

    if (i < session->numCommands) {
        session->commandProcs[i].function = callback ;
    } else if (session->numCommands < MAXCOMMANDS) {
        session->commandProcs[i].command = strdup (command) ;
        if (session->commandProcs[i].command == NULL) {
            LGE "(nftRegister) Error duplicating command name, \"%s\".\nstrdup: ",
                command) ;
            return (errno) ;
        }
        session->commandProcs[i].function = callback ;
        ...
        SET_ERRNO (ENOMEM) ;
        LGE "(nftRegister) Attempt to register more than %lu commands: ",
            (unsigned long) session->numCommands) ;
        return (errno) ;
    }
