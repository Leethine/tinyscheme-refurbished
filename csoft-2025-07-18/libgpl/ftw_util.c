/* $Id: ftw_util.c,v 1.11 2021/09/20 02:14:02 alex Exp alex $ */
/*******************************************************************************
...
#if (!defined(HAVE_GETGID) || HAVE_GETGID) &&	\
    (!defined(HAVE_GETUID) || HAVE_GETUID)
...
static  errno_t  ftwScanDirectory (
#    if PROTOTYPES
        const  char  *root,
...
/* Convert the wildcard file specification to a regular expression string. */
    ...
    if (wildcardRE == NULL) {
        LGE "(fileTreeWalk) Error converting \"%s\" to a wildcard RE.\nrexWild: ",
            fileSpec) ;
        return (errno) ;
    }
...
/* Compile a regular expression (RE) for the wildcard file specification. */
    ...
    if (rexCompile (wildcardRE, &pattern)) {
        LGE "(fileTreeWalk) Error compiling regular expression for \"%s\".\nrexCompile: ",
            fileSpec) ;
        PUSH_ERRNO ;  free (wildcardRE) ;  POP_ERRNO ;
        return (errno) ;
    }
...
/* Visit each file in the directory tree, invoking the file callback function
   for each file whose name matches the wildcard file specification. */
    ...
    status = ftwScanDirectory (root, pattern, flags,
...
Purpose:
    ...
    where:
        ...
        <directory>		- I
            is the name of the directory being scanned.
        <wildcardRE>		- I
...
{    /* Local variables. */
    char  *fileName, *fullFileName, *pathname ;
    DirectoryScan  scan ;
...
#endif
#if !defined(HAVE_GETUID) || HAVE_GETUID
    uid_t  userID = getuid () ;
#endif
...
/* Construct a list of all the files in the directory. */
    ...
    if (directory[strlen (directory) - 1] == '/')
        pathname = strDupCat (2, directory, "*") ;
    else
        pathname = strDupCat (2, directory, "/*") ;
    if (pathname == NULL) {
        LGE "(ftwScanDirectory) Error duplicating directory name: %s\nstrDupCat: ",
            directory) ;
        return (errno) ;
    }
...
/* If the wildcard file specification matches the current file name, then
   invoke the file callback function. */
        ...
        if ((fileType == FtwDirectory) ||
            (fileType == FtwDirNoRead) ||
            rexMatch (fileName, -1, wildcardRE, NULL, NULL, 0)) {
