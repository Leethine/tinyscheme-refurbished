/* $Id: wcs_util.c,v 1.5 2019/05/29 05:29:49 alex Exp $ */
/*******************************************************************************
...
#endif

#if defined(HAVE_WCSNDUP) && !HAVE_WCSNDUP
/*!*****************************************************************************
...
#endif

#if defined(HAVE_WCSNLEN) && !HAVE_WCSNLEN
/*!*****************************************************************************
...
Purpose:
    ...
    Function wcsnlen() determines the length of a wide-character string
    in a fixed-size buffer.  If the string is NUL-terminated within the
    buffer, wcsnlen() returns the NUL-terminated length of the string,
    just like wcslen() would do.  If the string is not NUL-terminated
    within the buffer, wcsnlen() returns the size of the buffer as the
    wide-character string length.


    Invocation:

        length = wcsnlen (wideString, maxLength) ;

    where:

        <wideString>	- I
            is the wide-character string whose length is to be determined.
        <maxLength>	- I
            is the maximum number of characters in the string to examine
            while looking for a wide-character NUL terminator.
        <length>	- O
            returns the number of wide characters preceding the NUL terminator
            if the NUL is found within MAXLENGTH characters; i.e., the length
            of the NUL-terminated string is less than or equal to MAXLENGTH-1
            wide characters.  Otherwise, MAXLENGTH is returned.
...
{    /* Local variables. */
    const  wchar_t  *w = wideString ;



    if (w == NULL) {
        errno = EINVAL ;
        LGE "(wcsnlen) NULL string: ") ;
        return (0) ;
    }

    while ((*w != 0) && (maxLength-- > 0))
        w++ ;
