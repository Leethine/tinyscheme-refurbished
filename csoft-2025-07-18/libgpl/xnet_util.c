/* $Id: xnet_util.c,v 1.25 2019/05/30 01:44:25 alex Exp alex $ */
/*******************************************************************************
...
/* The first and second arguments to the readit() and writeit() functions
   passed to xdrrec_create() are generic pointers.  Some XDR libraries
   declare the arguments as "char *" and other libraries declare them as
   "void *".  The widely used "char *" (or "caddr_t") appears to have
   predated the "void *" version, so assume the former.  On platforms
   where the XDR library uses "void *", simply compile this source
   file with a "-DXDRREC_CADDR_T=void*" option. */

#ifndef XDRREC_CADDR_T
#    define  XDRREC_CADDR_T  char *
#endif
...
#    if PROTOTYPES
        XDRREC_CADDR_T  stream,
        XDRREC_CADDR_T  buffer,
        int  numBytesToRead
#    endif
    ) ;
...
*******************************************************************************/
    ...
    *stream = (_XnetStream *) malloc (sizeof (_XnetStream)) ;
    ...
    xdrrec_create (&(*stream)->xdrStream, bufferSize, bufferSize,
                   (XDRREC_CADDR_T) *stream, xnetReadStream, xnetWriteStream) ;
    ...
    LGI "(xnetCreate) Created XDR-based network stream %s, socket %d\n",
        xnetName (*stream), (int) xnetFd (*stream)) ;
...
{    /* Local variables. */
    ...
    if (tcpRead (((XnetStream) stream)->connection,
                 ((XnetStream) stream)->timeout,
                 - ((ssize_t) numBytesToRead),
                 buffer, &numBytesRead)) {
        LGE "(xnetReadStream) Error reading %d bytes from %s stream.\ntcpRead: ",
            numBytesToRead, xnetName ((XnetStream) stream)) ;
        return (-1) ;
    }
