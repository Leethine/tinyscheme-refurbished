/* $Id: comx_util.c,v 1.21 2011/07/18 17:54:19 alex Exp alex $ */
/*******************************************************************************
...
Purpose:
    ...
    47,400 and 101,000 hits, respectively ...)
        ...
        NOTE:  As a big fan of Victorian novels at the time I wrote the
        codswallop above, I suppose I must have gotten so used to British
        English spelling that I failed to recognize American English
        spelling!  Ahh, the good old days -- when one wrote of "to-day",
        "to-morrow", and "e-mail"; when "just deserts" were just that;
        "careen" was not a synonym for "career"; and you could "discuss"
        a nice hot cup of cocoa and a digestive biscuit ..
...
typedef  struct  _ComxChannel {
    ...
    size_t  offset ;			/* Offset in octets of start of buffer. */
    octet  *buffer ;			/* GIOP buffer. */
    size_t  length ;			/* Length of GIOP buffer. */
...
/* Allocate a buffer, if necessary, for encoding. */
    ...
    LGI "(comxCreate) Created marshaling channel for %"PRIuSIZE_T"-octet buffer %p.\n",
...

{
    ...
    LGI "(comxDestroy) Closing channel for %"PRIuSIZE_T"-byte buffer %p ...\n",
        channel->length, channel->buffer) ;
...
{    /* Local variables. */
    octet  *newBuffer ;
    size_t  currentOffset, newLength ;
...
#define  CHUNK  1024
    ...
    LGI "(comxExtend) Increasing buffer size from %"PRIuSIZE_T" to %"PRIuSIZE_T" octets.\n",
...
/* Allocate a new buffer of increased size and copy over the contents of
   the old buffer. */
    ...
    if (channel->buffer == NULL) {		/* Channel's first buffer? */
        ...
        if (newBuffer == NULL) {
            LGE "(comxExtend) Error allocating %"PRIuSIZE_T"-octet buffer.\ncalloc: ",
                newLength) ;
            return (errno) ;
        }
        ...
        if (newBuffer == NULL) {
            LGE "(comxExtend) Error reallocating %"PRIuSIZE_T"-octet buffer.\nrealloc: ",
                newLength) ;
            return (errno) ;
        }
    }
...
{    /* Local variables. */
    ...
    case MxENCODE:
        RETURN_IF_NULL (value) ;
        s = *value ;
        length = (s == NULL) ? 0 : (unsigned long) (strlen (s) + 1) ;
        CHECK (comxULong (channel, &length)) ;
...
{    /* Local variables. */
    ...
    case MxENCODE:
        ...
        saveLE = channel->isLE ;
        ws = *value ;
        length = (ws == NULL) ? 0 : (unsigned long) wcslen (ws) ;
...
{    /* Local variables. */
    ...
    case MxENCODE:
        RETURN_IF_NULL (value) ;
        count = (unsigned long) value->count ;
        CHECK (comxULong (channel, &count)) ;
