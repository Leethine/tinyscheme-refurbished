/* $Id: tcp_util.c,v 1.32 2019/05/30 01:35:04 alex Exp alex $ */
/*******************************************************************************
...
/* Wait for a connection request from a client. */
    ...
    for ( ; ; ) {
        ...
        numActive = select ((int) listeningPoint->fd+1, &readMask, NULL, NULL,
...
/* Return the data connection to the caller. */
    ...
    LGI "(tcpAnswer) Accepted connection %s, socket %ld.\n",
        (*dataPoint)->name, (long) (*dataPoint)->fd) ;
...
/* Lookup the port number bound to the server name. */
    ...
    if (netPortOf (serverName, "tcp", &portNumber)) {
...
/* Bind the network address to the socket and enable it to listen for
   connection requests. */
    ...
    LGI "(tcpListen) Listening on %s, port %u, socket %ld.\n",
        (*listeningPoint)->name, portNumber, (long) (*listeningPoint)->fd) ;
...
/* Poll the listening socket for input. */
    ...
    for ( ; ; ) {
        ...
        if (select ((int) listeningPoint->fd+1,
                    &readMask, NULL, NULL,
                    &timeout) >= 0)
            break ;
        SET_ERRNO (GET_NETERRNO ()) ;
        if (errno == EINTR)  continue ;		/* Retry on signal interrupt. */
        LGE "(tcpRequestPending) Error polling endpoint %s, socket %ld.\nselect: ",
            listeningPoint->name, (long) listeningPoint->fd) ;
        return (false) ;
    }
...
{    /* Local variables. */
    char  *s, hostName[MAXHOSTNAMELEN+1], portName[MAXHOSTNAMELEN+1] ;
    in_port_t  portNumber ;
    IoctlArg  optval ;
    struct  sockaddr_in  socketName ;
    socklen_t  length ;
...
/* Parse the host and server names.  If the host name is not defined
   explicitly, it defaults to the local host. */
    ...
    s = strchr (serverName, '@') ;
    if (s == NULL) {				/* "<server>" */
        strcpy (portName, serverName) ;
    } else {					/* "<server>@<host>" */
        length = (socklen_t) (s - serverName) ;
        strncpym (portName, serverName, length, sizeof portName) ;
        strcpy (hostName, ++s) ;
    }
...
/* Create an endpoint structure. */
    ...
    *dataPoint = (TcpEndpoint) malloc (sizeof (_TcpEndpoint)) ;
    ...
    (*dataPoint)->name = malloc (strlen (portName) + 1 +
                                 strlen (hostName) + 1) ;
    if ((*dataPoint)->name == NULL) {
    ...
    sprintf ((*dataPoint)->name, "%s@%s", portName, hostName) ;


/* Lookup the port number bound to the server name. */

    if (netPortOf (portName, "tcp", &portNumber)) {
...
/* The connection has been established.  Configure the socket so that the
   ...
   by "pinging" the server. */
    ...
    LGI "(tcpCall) Connected to %s, port %u, socket %ld.\n",
        (*dataPoint)->name, sktPort ((*dataPoint)->fd),
        (long) (*dataPoint)->fd) ;
...
/* Wait for the call to complete, which is indicated by the connection
   being writeable.  */
    ...
    for ( ; ; ) {
        ...
        numActive = select ((int) dataPoint->fd+1, NULL, &writeMask, NULL,
                            (timeout < 0.0) ? NULL : &deltaTime) ;
        if (numActive > 0)  break ;
        SET_ERRNO (GET_NETERRNO ()) ;
...
/* Reconfigure the socket for blocking I/O. */
    ...
    LGI "(tcpComplete) Connected to %s, port %u, socket %ld.\n",
        dataPoint->name, sktPort (dataPoint->fd), (long) dataPoint->fd) ;
...
/* Wait for data to arrive. */
        ...
        for ( ; ; ) {
            ...
            numActive = select ((long) dataPoint->fd+1, &readMask, NULL, NULL,
                                &deltaTime) ;
            if (numActive >= 0)  break ;
            SET_ERRNO (GET_NETERRNO ()) ;
            if (errno == EINTR)  continue ;	/* Retry on signal interrupt. */
            LGE "(tcpRead) Error waiting for input on %s, socket %ld.\nselect: ",
                dataPoint->name, (long) dataPoint->fd) ;
            return (errno) ;
        }
...
*******************************************************************************/
    ...
    while (numBytesToRead > 0) {
        ...
        if (length < 0) {
            ...
            SET_ERRNO (EPIPE) ;
            LGE "(tcpRead) Broken connection on %s, socket %ld.\nread: ",
                dataPoint->name, (long) dataPoint->fd) ;
            return (errno) ;
        }

        LGI "(tcpRead) Read %"PRIdSSIZE_T" bytes from %s, socket %ld.\n",
            length, dataPoint->name, (long) dataPoint->fd) ;
        if (I_DEFAULT_GUARD) {
...
/* Write the next chunk of data to the network. */
        ...
        LGI "(tcpWrite) Wrote %"PRIdSSIZE_T" bytes to %s, socket %ld.\n",
            length, dataPoint->name, (long) dataPoint->fd) ;
        if (I_DEFAULT_GUARD) {
            meoDumpX (NULL, "    ", 0, buffer, length) ;
        }
...

{
    ...
    LGI "(tcpDestroy) Closing %s, socket %ld.\n",
        (endpoint->name == NULL) ? "<null>" : endpoint->name,
        (long) endpoint->fd) ;
