/* $Id: udp_util.c,v 1.19 2019/05/30 01:37:58 alex Exp alex $ */
/*******************************************************************************
...
/* Parse the host and server names.  If the host name is not defined
   explicitly, it defaults to the local host. */
    ...
    if (serverName == NULL) {			/* Let system choose port #? */
        strcpy (portName, "0") ;
...
/* Lookup the port number bound to the server name. */
    ...
    if (netPortOf (portName, "udp", &portNumber)) {
        LGE "(udpCreate) Error getting server entry for %s%c%s.\nnetPortOf: ",
            portName, (parent == NULL) ? '#' : '@', hostName) ;
        return (errno) ;
    }
...
*******************************************************************************/
    ...
    *endpoint = (UdpEndpoint) malloc (sizeof (_UdpEndpoint)) ;
    if (*endpoint == NULL) {
        LGE "(udpCreate) Error allocating endpoint structure for %s%c%s.\nmalloc: ",
            portName, (parent == NULL) ? '#' : '@', hostName) ;
        return (errno) ;
    }
...
/* Create a socket for the endpoint. */
        ...
        if (!VALID_FD ((*endpoint)->fd)) {
            ...
            LGE "(udpCreate) Error creating socket for %s%c%s.\nsocket: ",
                portName, (parent == NULL) ? '#' : '@', hostName) ;
            PUSH_ERRNO ;
...
/* Bind the network address to the socket. */
        ...
        if (bind ((*endpoint)->fd,
            ...
            SET_ERRNO (GET_NETERRNO ()) ;
            LGE "(udpCreate) Error binding %s%c%s's socket name.\nbind: ",
                portName, (parent == NULL) ? '#' : '@', hostName) ;
            PUSH_ERRNO ;
            udpDestroy (*endpoint) ;  *endpoint = NULL ;
            POP_ERRNO ;
        ...
        sprintf (portName, "%hu", sktPort ((*endpoint)->fd)) ;
...
*******************************************************************************/
    ...
    (*endpoint)->name = malloc (strlen (portName) + 1 +
                                strlen (hostName) + 1) ;
    ...
    LGI "(udpCreate) Created %s, socket %ld.\n",
        (*endpoint)->name, (long) (*endpoint)->fd) ;
...
{    /* Local variables. */
    ...
    LGI "(udpDestroy) Destroying %s, socket %ld ...\n",
        endpoint->name, (long) endpoint->fd) ;
...
/* Wait for the next message to arrive. */
        ...
        for ( ; ; ) {
            ...
            numActive = select ((int) fd+1, &readMask, NULL, NULL, &deltaTime) ;
...
/* If not, create a brand new endpoint.  If so, then use the existing one. */
    ...
    LGI "(udpRead) Read %ld bytes from %s on %s, socket %ld.\n",
        (long) length, sourcePoint->name, endpoint->name, (long) endpoint->fd) ;
    if (udp_util_debug) {
...
/* Wait for the endpoint to be ready for writing. */
        ...
        for ( ; ; ) {
            ...
            numActive = select ((int) fd+1, NULL, &writeMask, NULL,
                                &deltaTime) ;
            if (numActive >= 0)  break ;
            SET_ERRNO (GET_NETERRNO ()) ;
            if (errno == EINTR)  continue ;
...
*******************************************************************************/
    ...
    LGI "(udpWrite) Wrote %d bytes to %s, socket %ld.\n",
        length, destination->name, (long) fd) ;
    if (udp_util_debug) {
        meoDumpX (stdout, "    ", 0, buffer, length) ;
    }
