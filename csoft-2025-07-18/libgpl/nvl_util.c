/* $Id: nvl_util.c,v 1.9 2021/04/21 01:50:37 alex Exp alex $ */
/*******************************************************************************
...
typedef  struct  _NVList {
    ...
    size_t  maxMembers ;		/* Maximum # of pairs in list. */
    size_t  numMembers ;		/* Actual # of pairs in list. */
...
/* If there isn't enough room for the new member in the pair array,
   then expand the array. */
    ...
    if (list->numMembers >= list->maxMembers) {
        ...
        if (array == NULL) {
            LGE "(nvlAdd) Error expanding %s list to %lu members.\nrealloc: ",
                nvlName (list), (unsigned long) newMax) ;
            return (errno) ;
        }
        list->pairs = array ;
...
/* Print each name/value pair in the list. */
    ...
    for (i = 0 ;  i < nvlCount (list) ;  i++) {
        ...
        if (nvpTypeOf (pair) == NvpList) {
            char  *s = strndupx (indent, strlen (indent), 4) ;
            strcat (s, "    ") ;
            array = nvpValue (pair) ;
