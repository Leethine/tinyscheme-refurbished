/* $Id: nicl.c,v 1.3 2023/05/27 12:42:22 alex Exp alex $ */
/*******************************************************************************
...
Purpose:
    ...
    Invocation:
        ...
        % nicl [-daemon] [-debug] [-Debug] [-help]
               [-evaluate <code>] [-listen <port>] [<file(s)>]

    where:

        "-daemon"
            causes NICL to become a daemon process.
        "-debug"
        "-Debug"
...
{    /* Local variables. */
    ...
    const  char  *optionList[] = {	/* Command line options. */
        "{daemon}", "{Debug}", "{debug}", "{help}",
        "{evaluate:}", "{listen:}", NULL
    } ;
...
*******************************************************************************/
    ...
    if (errflg || help || (server == NULL)) {
        fprintf (stderr, "Usage:  nicl [-debug] [-Debug] [-help]\n") ;
        fprintf (stderr, "             [-daemon] [-evaluate <code>] [-listen <port>] [<file(s)>]\n") ;
        exit (EINVAL) ;
    }

/*******************************************************************************
    If NICL is to run as a daemon, then make it a daemon.  The daemon's
