/* $Id: funcs_rex.c,v 1.1 2009/09/01 11:31:46 alex Exp alex $ */
/*******************************************************************************
...
/* Compile the regular expression. */
    ...
    if (rexCreate (regexp, &pattern, NULL)) {
...
/* Destroy the compiled regular expression. */
    ...
    return (rexDestroy (pattern, NULL) ? sc->F : sc->T) ;
...

{
    ...
    return (sc->F) ;	/* Not currently accessible in the (un)compiled RE. */
...
Purpose:
    ...
    target string.
        ...
        subexpresions (capture groups 1..N), if any, in the regular expression:

            ("<full match>" "<subexp1>" "<subexp2>" ...))
...
{    /* Local variables. */
    char  *target ;
    const  char  *groupStart, *matchStart ;
    CompiledRE  pattern ;
    MatchedRE  match ;
    size_t  groupLength, matchLength, numGroups ;
...
/* Create a matched RE structure for the regular expression's capture groups.
   The fields will be filled in by the rexMatch() call. */

    if (rexCreate (NULL, NULL, &match)) {
        LGE "(func_REX_MATCH) Error creating matched RE struture.\nrexCreate: ") ;
        return (sc->F) ;
    }

/* Attempt to match the regular expression against the target string. */

    if (!rexMatch ((const char *) target, -1, pattern, match,
                   &matchStart, &matchLength)) {
        rexDestroy (NULL, match) ;
...
/* The match was successful.  Construct a Scheme list whose car is a string
   ...
   subexpressions. */
    ...
    while (numGroups-- > 0) {		/* Build list in reverse. */
        rexGetGroup (match, numGroups, &groupStart, &groupLength) ;
        cdr (pair) = cons (sc,
                           (groupStart == NULL) ?
                               sc->F :
                               mk_bstring (sc, groupStart, groupLength),
                           cdr (pair)) ;
    }
...
{    /* Local variables. */
    ...
    ssize_t  length, maxSubstitutions ;
...
/* Get the argument(s). */
    ...
    argument = car (args) ;
    if (is_string (argument)) {
        source = (const char *) strvalue (argument) ;
        length = (ssize_t) strlength (argument) ;
    } else {
        SET_ERRNO (EINVAL) ;
        LGE "(func_REX_REPLACE) Text argument is not a string: ") ;
        return (sc->F) ;
    }
    ...
    argument = car (args) ;
    if (is_string (argument)) {
        replacement = (const char *) strvalue (argument) ;
    } else {
        SET_ERRNO (EINVAL) ;
        LGE "(func_REX_REPLACE) Replacement argument is not a string: ") ;
        return (sc->F) ;
    }
    ...
    if (isInteger (argument)) {
        maxSubstitutions = (ssize_t) ivalue (argument) ;
    } else if (argument == sc->T) {
        maxSubstitutions = -1 ;
    } else {
        maxSubstitutions = 1 ;
    }
...
/* Perform the search-and-replace operation on the source string. */
    ...
    if (rexReplace (source, length, pattern, replacement, maxSubstitutions,
...
/* The operation was successfully completed.  Return a Scheme pair whose car
   ...
   that were made. */
    ...
    pair = cons (sc, mk_string (sc, result),
