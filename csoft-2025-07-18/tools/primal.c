/* $Id: primal.c,v 1.5 2021/09/16 01:44:46 alex Exp alex $ */
/*******************************************************************************
...
Program:
    ...
    PRIMe Algorithm tester Lite (PRIMAL)
...
Purpose:
    ...
    PRIMAL does various things with prime numbers.  It's heavyweight
    sibling, PRIMATE, was written to test my prime-number functions
    in LIBGPL's NBR_UTIL package.  This lightweight version strips a
    lot of the comparative functionality, specifically the ability
    to compare benchmark times for different prime number functions.
    My hope is that PRIMAL will run on my Palm Pilot (emulator) --
    the emulator has an immediate, fatal error when SioMain() calls
    PRIMATE's main() function.
    ...
    good bit of time.  Pre-computed tables contain the primes in the 3 ranges.
        ...
        % primal -verify last		# Last 1000 32- or 64-bit prime numbers.
        % primal -verify last32		# Last 1000 32-bit prime numbers.
        % primal -verify last64		# Last 1000 64-bit prime numbers.
    ...
    Select a different skip-through prime for the nbrIsPrime() function; the
    default is STP 7.  This option also generates the sequence of differences
    for the new skip-through prime; the often lengthy time taken to generate
    a sequence is not included in timing benchmarks ("-time").
...
*******************************************************************************/
    ...
    config = nbrPrimeConfigCreate (7, NULL, false) ;
    ...
    while ((option = opt_get (scan, &argument)) && !errflg) {
        ...
        case 10:			/* "-qmax <prime>" */
            ...
            subop = "maximum" ;
            break ;
        case 11:			/* "-qprime <prime>" */
            skipThroughPrime = STRTO_PRIME_T (argument, &s, 0) ;
            if (*s != '\0') {
                errflg++ ;  break ;
            }
            length = nbrPrimeSeqDiffsLength (skipThroughPrime,
                                             &skipThroughPrime, NULL) ;
            if (length == 0) {
                errflg++ ;  break ;
            }
            printf ("Skip-Through Prime: %"PRIuPRIME_T"  Sequence Length: %"PRIuSIZE_T"  Generating ... ",
                    skipThroughPrime, length) ;
            fflush (stdout) ;
            config = nbrPrimeConfigCreate (number, NULL, false) ;
            printf ("Done\n") ;
            if (config == NULL)  errflg++ ;
            break ;
        case 12:			/* "-repeat <count>" */
            repeat = (size_t) strtoul (argument, &s, 0) ;
            if (*s != '\0')  errflg++ ;
            break ;
        case 13:			/* "-time" */
            benchmark = true ;
            break ;
        case 14:			/* "-verify first|last|last32|last64" */
            if (operation != NULL) {	/* Operation already specified? */
               errflg++ ;  break ;
            }
            strlwr (argument) ;
...
*******************************************************************************/
    ...
    if (help) {
        ...
        fprintf (stderr, "    sizeof (prime_t) = %"PRIuSIZE_T" bytes\n",
                 sizeof (prime_t)) ;
        fprintf (stderr, "    Range: 0 ... %"PRIuPRIME_T" (PRIME_T_MAX)\n", PRIME_T_MAX) ;
        fprintf (stderr, "\n") ;
        fprintf (stderr, "Operations:\n") ;
        fprintf (stderr, "\n") ;
        fprintf (stderr, "    %s <number>\n", program) ;
