/* $Id: colior.c,v 1.4 2019/05/31 01:40:33 alex Exp $ */
/*******************************************************************************
...
{    /* Local variables. */
    ...
    static  const  char  *testIOR =		/* Sample IOR for testing. */
    ...
    for (i = 0 ;  i < ior.profiles.count ;  i++) {
        ...
        if (profile->which == IOP_TAG_INTERNET_IOP) {
            ...
            for (j = 0 ;  j < body->components.count ;  j++) {
                ...
                if (component->tag == IOP_TAG_ORB_TYPE) {
                    unsigned  long  orbType ;
                    if (comxEncapsule (body->iiop_version, MxDECODE,
                        &component->component_data,
                        comxULong, &orbType, NULL)) {
                        LGE "[%s] Error decoding encapsulated code information.\ncomxEncapsule: ",
                            argv[0]) ;
                    } else {
                        printf ("         ORB Type:  0x%lX (%lu)\n",
                                orbType, orbType) ;
                    }
                } else if (component->tag == IOP_TAG_CODE_SETS) {
