...
const char * INITFILE_BLOB1 = "(define (caar x) (car (car x))) "
...
const char * INITFILE_BLOB2 = "(define (cdaaar x) (cdr (car (car (car x))))) "
...
const char * INITFILE_BLOB3 = "(macro (define-macro dform) "
...
const char * INITFILE_BLOB4 = "(define (min . lst) "
"  (foldr (lambda (a b) "
"           (if (< a b) "
"             (if (exact? b) a (+ a 0.0)) "
"             (if (exact? a) b (+ b 0.0)))) "
...
const char * INITFILE_BLOB5 = "(define (string . charlist) "
...
const char * INITFILE_BLOB6 = "(define (string-fill! s e) "
...
const char * INITFILE_BLOB7 = "(define (string->number str . base) "
...
const char * INITFILE_BLOB8 = "(define (string-cmp? chcmp cmp a b) "
...
const char * INITFILE_BLOB9 = "(define (list . x) x) "
...
const char * INITFILE_BLOB10 = "(define (map proc . lists) "
"  (if (null? lists) "
"      (apply proc) "
"      (if (null? (car lists)) "
"        '() "
...
const char * INITFILE_BLOB11 = "(define (head stream) (car stream)) "
...
const char * INITFILE_BLOB12 = "(macro "
" quasiquote "
" (lambda (l) "
"   (define (mcons f l r) "
"     (if (and (pair? r) "
...
const char * INITFILE_BLOB13 = "(define (shared-tail x y) "
"   (let ((len-x (length x)) "
"         (len-y (length y))) "
...
const char * INITFILE_BLOB14 ="(unless (defined? 'dynamic-wind) "
"   (let "
"      (  (*active-windings* '()) "
...
const char * INITFILE_BLOB15 = "(define call/cc call-with-current-continuation) "
"(define (atom? x) "
"  (not (pair? x))) "
"(define (equal? x y) "
"     (cond "
...
const char * INITFILE_BLOB16 = "(define (generic-member cmp obj lst) "
"  (cond "
"    ((null? lst) #f) "
"    ((cmp obj (car lst)) lst) "
...
const char * INITFILE_BLOB17 = "(macro (make-environment form) "
"     `(apply (lambda () "
"               ,@(cdr form) "
"               (current-environment)))) "
...
const char * INITFILE_BLOB18 =  "(define (with-output-to-file s p) "
"     (let ((outport (open-output-file s))) "
"          (if (eq? outport #f) "
"               #f "
...
const char * INITFILE_BLOB19 = "(define (cond-expand-runtime cond-action-list) "
  ...
  do {
    ...
    if(isfile && fin==0) {
      if (file_name && strcmp(file_name, InitFile) == 0) {
          // INIT LOAD HERE
        scheme_load_string(&sc, INITFILE_BLOB1);
        scheme_load_string(&sc, INITFILE_BLOB2);
        scheme_load_string(&sc, INITFILE_BLOB3);
        scheme_load_string(&sc, INITFILE_BLOB4);
        scheme_load_string(&sc, INITFILE_BLOB5);
        scheme_load_string(&sc, INITFILE_BLOB6);
        scheme_load_string(&sc, INITFILE_BLOB7);
        scheme_load_string(&sc, INITFILE_BLOB8);
        scheme_load_string(&sc, INITFILE_BLOB9);
        scheme_load_string(&sc, INITFILE_BLOB10);
        scheme_load_string(&sc, INITFILE_BLOB11);
        scheme_load_string(&sc, INITFILE_BLOB12);
        scheme_load_string(&sc, INITFILE_BLOB13);
        scheme_load_string(&sc, INITFILE_BLOB14);
        scheme_load_string(&sc, INITFILE_BLOB15);
        scheme_load_string(&sc, INITFILE_BLOB16);
        scheme_load_string(&sc, INITFILE_BLOB17);
        scheme_load_string(&sc, INITFILE_BLOB18);
        scheme_load_string(&sc, INITFILE_BLOB19);
      }
      else {
