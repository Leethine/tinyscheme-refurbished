...
const char * INITBLOB41 =  " (define (shared-tail x y) (let ((len-x (length x)) (len-y (length y))) (define (shared-tail-helper x y) (if (eq? x y) x (shared-tail-helper (cdr x) (cdr y)))) " ;
const char * INITBLOB42 =  "  (cond ((> len-x len-y) (shared-tail-helper (list-tail x (- len-x len-y)) y)) ((< len-x len-y) (shared-tail-helper x (list-tail y (- len-y len-x)))) (#t (shared-tail-helper x y))))) " ;
const char * INITBLOB43 =  " (unless (defined? 'dynamic-wind) (let ( (*active-windings* '()) (outer-env (current-environment))) " ;
const char * INITBLOB44 =  "  (define before-func car) (define after-func cdr) (define make-winding cons) " ;
const char * INITBLOB45 =  "  (define (activate-winding! new) ((before-func new)) (set! *active-windings* (cons new *active-windings*))) (define (deactivate-top-winding!) (let ((old-top (car *active-windings*))) (set! *active-windings* (cdr *active-windings*)) ((after-func old-top)))) " ;
const char * INITBLOB46 =  "  (define (set-active-windings! new-ws) (unless (eq? new-ws *active-windings*) (let ((shared (shared-tail new-ws *active-windings*))) " ;
const char * INITBLOB47 =  "  (define (pop-many) (unless (eq? *active-windings* shared) (deactivate-top-winding!) (pop-many))) (define (push-many new-ws) (unless (eq? new-ws shared) (push-many (cdr new-ws)) (activate-winding! (car new-ws)))) " ;
const char * INITBLOB48 =  "  (pop-many) (push-many new-ws)))) " ;
const char * INITBLOB49 =  "  (eval `(define call-with-current-continuation ,(let ((old-c/cc call-with-current-continuation)) (lambda (func) (old-c/cc (lambda (continuation) (func (let ((current-ws *active-windings*)) (lambda (x) (set-active-windings! current-ws) (continuation x))))))))) outer-env) (eval `(define dynamic-wind ,(lambda (before thunk after) (activate-winding! (make-winding before after)) (let ((result (thunk))) (deactivate-top-winding!) result))) outer-env))) " ;
const char * INITBLOB50 =  " (define call/cc call-with-current-continuation) " ;
const char * INITBLOB51 =  " (define (atom? x) (not (pair? x))) " ;
const char * INITBLOB52 =  " (define (equal? x y) (cond ((pair? x) (and (pair? y) (equal? (car x) (car y)) (equal? (cdr x) (cdr y)))) ((vector? x) (and (vector? y) (vector-equal? x y))) ((string? x) (and (string? y) (string=? x y))) (else (eqv? x y)))) " ;
const char * INITBLOB53 =  " (macro do (lambda (do-macro) (apply (lambda (do vars endtest . body) (let ((do-loop (gensym))) `(letrec ((,do-loop (lambda ,(map (lambda (x) (if (pair? x) (car x) x)) `,vars) (if ,(car endtest) (begin ,@(cdr endtest)) (begin ,@body (,do-loop ,@(map (lambda (x) (cond ((not (pair? x)) x) ((< (length x) 3) (car x)) (else (car (cdr (cdr x)))))) `,vars))))))) (,do-loop ,@(map (lambda (x) (if (and (pair? x) (cdr x)) (car (cdr x)) '())) `,vars))))) do-macro))) " ;
const char * INITBLOB54 =  " (define (generic-member cmp obj lst) (cond ((null? lst) #f) ((cmp obj (car lst)) lst) (else (generic-member cmp obj (cdr lst))))) " ;
const char * INITBLOB55 =  " (define (memq obj lst) (generic-member eq? obj lst)) (define (memv obj lst) (generic-member eqv? obj lst)) (define (member obj lst) (generic-member equal? obj lst)) " ;
const char * INITBLOB56 =  " (define (generic-assoc cmp obj alst) (cond ((null? alst) #f) ((cmp obj (caar alst)) (car alst)) (else (generic-assoc cmp obj (cdr alst))))) " ;
const char * INITBLOB57 =  " (define (assq obj alst) (generic-assoc eq? obj alst)) (define (assv obj alst) (generic-assoc eqv? obj alst)) (define (assoc obj alst) (generic-assoc equal? obj alst)) " ;
const char * INITBLOB58 =  " (define (acons x y z) (cons (cons x y) z)) " ;
const char * INITBLOB59 =  " (macro (define-with-return form) `(define ,(cadr form) (call/cc (lambda (return) ,@(cddr form))))) " ;
const char * INITBLOB60 =  " (define *handlers* (list)) " ;
const char * INITBLOB61 =  " (define (push-handler proc) (set! *handlers* (cons proc *handlers*))) " ;
const char * INITBLOB62 =  " (define (pop-handler) (let ((h (car *handlers*))) (set! *handlers* (cdr *handlers*)) h)) " ;
const char * INITBLOB63 =  " (define (more-handlers?) (pair? *handlers*)) " ;
const char * INITBLOB64 =  " (define (throw . x) (if (more-handlers?) (apply (pop-handler)) (apply error x))) " ;
const char * INITBLOB65 =  " (macro (catch form) (let ((label (gensym))) `(call/cc (lambda (exit) (push-handler (lambda () (exit ,(cadr form)))) (let ((,label (begin ,@(cddr form)))) (pop-handler) ,label))))) " ;
const char * INITBLOB66 =  " (define *error-hook* throw) " ;
const char * INITBLOB67 =  " (macro (make-environment form) `(apply (lambda () ,@(cdr form) (current-environment)))) " ;
const char * INITBLOB68 =  " (define-macro (eval-polymorphic x . envl) (display envl) (let* ((env (if (null? envl) (current-environment) (eval (car envl)))) (xval (eval x env))) (if (closure? xval) (make-closure (get-closure-code xval) env) xval))) " ;
const char * INITBLOB69 =  " (define *colon-hook* eval) " ;
const char * INITBLOB70 =  " (define (input-output-port? p) (and (input-port? p) (output-port? p))) " ;
const char * INITBLOB71 =  " (define (close-port p) (cond ((input-output-port? p) (close-input-port p) (close-output-port p)) ((input-port? p) (close-input-port p)) ((output-port? p) (close-output-port p)) (else (throw \"Not a port\" p)))) " ;
const char * INITBLOB72 =  " (define (call-with-input-file s p) (let ((inport (open-input-file s))) (if (eq? inport #f) #f (let ((res (p inport))) (close-input-port inport) res)))) " ;
const char * INITBLOB73 =  " (define (call-with-output-file s p) (let ((outport (open-output-file s))) (if (eq? outport #f) #f (let ((res (p outport))) (close-output-port outport) res)))) " ;
const char * INITBLOB74 =  " (define (with-input-from-file s p) (let ((inport (open-input-file s))) (if (eq? inport #f) #f (let ((prev-inport (current-input-port))) (set-input-port inport) (let ((res (p))) (close-input-port inport) (set-input-port prev-inport) res))))) " ;
const char * INITBLOB75 =  " (define (with-output-to-file s p) (let ((outport (open-output-file s))) (if (eq? outport #f) #f (let ((prev-outport (current-output-port))) (set-output-port outport) (let ((res (p))) (close-output-port outport) (set-output-port prev-outport) res))))) " ;
const char * INITBLOB76 =  " (define (with-input-output-from-to-files si so p) (let ((inport (open-input-file si)) (outport (open-input-file so))) (if (not (and inport outport)) (begin (close-input-port inport) (close-output-port outport) #f) (let ((prev-inport (current-input-port)) (prev-outport (current-output-port))) (set-input-port inport) (set-output-port outport) (let ((res (p))) (close-input-port inport) (close-output-port outport) (set-input-port prev-inport) (set-output-port prev-outport) res))))) " ;
const char * INITBLOB77 =  " (define *seed* 1) (define (random-next) (let* ((a 16807) (m 2147483647) (q (quotient m a)) (r (modulo m a))) (set! *seed* (- (* a (- *seed* (* (quotient *seed* q) q))) (* (quotient *seed* q) r))) (if (< *seed* 0) (set! *seed* (+ *seed* m))) *seed*)) " ;
const char * INITBLOB78 =  " (define *features* '(srfi-0 tinyscheme)) " ;
const char * INITBLOB79 =  " (define-macro (cond-expand . cond-action-list) (cond-expand-runtime cond-action-list)) " ;
const char * INITBLOB80 =  " (define (cond-expand-runtime cond-action-list) (if (null? cond-action-list) #t (if (cond-eval (caar cond-action-list)) `(begin ,@(cdar cond-action-list)) (cond-expand-runtime (cdr cond-action-list))))) " ;
const char * INITBLOB81 =  " (define (cond-eval-and cond-list) (foldr (lambda (x y) (and (cond-eval x) (cond-eval y))) #t cond-list)) " ;
const char * INITBLOB82 =  " (define (cond-eval-or cond-list) (foldr (lambda (x y) (or (cond-eval x) (cond-eval y))) #f cond-list)) " ;
const char * INITBLOB83 =  " (define (cond-eval condition) (cond ((symbol? condition) (if (member condition *features*) #t #f)) ((eq? condition #t) #t) ((eq? condition #f) #f) (else (case (car condition) ((and) (cond-eval-and (cdr condition))) ((or) (cond-eval-or (cdr condition))) ((not) (if (not (null? (cddr condition))) (error \"cond-expand : 'not' takes 1 argument\") (not (cond-eval (cadr condition))))) (else (error \"cond-expand : unknown operator\" (car condition))))))) " ;
const char * INITBLOB84 =  " (gc-verbose #f) " ;
#define INIT_BLOB_CONST_CHAR_DECLARED_HERE
...
int main(int argc, char **argv) {
  ...
  do {
    ...
    if(isfile && fin==0) {
      if (file_name && strcmp(file_name, InitFile) == 0) {

// init.scm blob loaded
...
scheme_load_string(&sc, INITBLOB77);
scheme_load_string(&sc, INITBLOB78);
scheme_load_string(&sc, INITBLOB79);
scheme_load_string(&sc, INITBLOB80);
scheme_load_string(&sc, INITBLOB81);
scheme_load_string(&sc, INITBLOB82);
scheme_load_string(&sc, INITBLOB83);
scheme_load_string(&sc, INITBLOB84);
#define INIT_BLOB_CONST_CHAR_LOADED_HERE
